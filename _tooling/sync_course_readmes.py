...
def _fix_clone_urls(text: str, folder: str) -> str:
    ...
    # Keep fence body flush-left — leading spaces break copy-paste in ```bash blocks.
    text = re.sub(
...
def _normalize_quickstart_cmds(cmds: list[str], folder: str) -> list[str]:
    """Force monorepo clone layout; strip accidental leading whitespace."""
    out: list[str] = []
    standalone_clone = re.compile(
        rf"^git clone https://github\.com/alexar76/{re.escape(folder)}\.git\s*$"
    )
    standalone_cd = re.compile(rf"^cd {re.escape(folder)}\s*$")
    for raw in cmds:
        cmd = raw.strip()
        if standalone_clone.match(cmd):
            cmd = f"git clone {GITHUB_CLONE}"
        elif standalone_cd.match(cmd):
            cmd = f"cd aimarket-courses/{folder}"
        out.append(cmd)
    return out
...
def _quickstart_block(folder: str) -> str:
    ...
    cmds = _normalize_quickstart_cmds(cmds, folder)
    # Flush-left inside the fence; do not indent the fence itself.
    body = "\n".join(cmds)
    return f"```bash\n{body}\n```"
...
def _refresh_quickstart_section(text: str, folder: str) -> str:
    ...
    # Match both flush and legacy 8-space-indented Quick start blocks.
    pattern = re.compile(
        r"(?:^|\n)[ \t]*## Quick start\s*\n\s*```bash[\s\S]*?```",
        re.MULTILINE,
    )
