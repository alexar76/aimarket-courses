...
def main() -> int:
    p = argparse.ArgumentParser(description="Run GAIA · ATLAS · Physical World Sensors exercises")
    ...
    p.add_argument("--lang", default="en", choices=("en", "ru", "es", "fr", "zh"))
    p.add_argument("-o", "--out", type=pathlib.Path, default=pathlib.Path("certificate.html"))
    p.add_argument("--skip-check", action="store_true", help=argparse.SUPPRESS)  # requires ACADEMY_ALLOW_SKIP_CHECK=1
    args = p.parse_args()
