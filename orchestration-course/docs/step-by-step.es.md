...
7. [M5 — State, memoria y contexto](#m5--state-memoria-y-contexto)
8. [M6 — Guardrails y seguridad](#m6--guardrails-y-seguridad)
9. [M7 — Confianza, verificación y provenance](#m7--confianza-verificación-y-provenance)
10. [M8 — Economía de la orquestación (avanzado)](#m8--economía-de-la-orquestación-avanzado)
11. [Ejercicios y certificado](#ejercicios-y-certificado)
12. [M9 — Observabilidad en cada lab](#m9--observabilidad-en-cada-lab)
13. [Puente: del curso a AI-Factory](#puente-del-curso-a-ai-factory)
14. [Lista de autocomprobación](#lista-de-autocomprobación)
...
pip install -e ".[dev]"
# M4/M7 (hub-lite) y M8 (sandbox):
pip install -e ".[hub-lite,sandbox,dev]"
pytest -q
export COURSE_LANG=es
python labs/lab01_agent_and_tool.py
```

**Colab:** [sitio del curso](https://alexar76.github.io/aimarket-courses/orchestration-course/) → Open in Colab → celda setup → `COURSE_LANG=es`.
...
**Lab:** `lab04_discover_invoke.py` · **~30 min** · requiere `[hub-lite]`

1. `pip install -e ".[hub-lite,dev]"`
2. Ejecuta el lab (espera ~10 s al arrancar hub).
3. discover → invoke → resultado.
4. Lectura: [hub-integration-guide.md](https://github.com/alexar76/aicom/blob/main/docs/hub-integration-guide.md)

---

## M5 — State, memoria y contexto

**Lab:** `lab05_state_context.py` · **~20 min**

1. Ejecuta el lab.
2. Observa cómo el `Context` (bill of materials) se pasa entre etapas del pipeline.
3. Comprobación: `python labs/run_exercises.py --module m5`

---

## M6 — Guardrails y seguridad

**Lab:** `lab06_guardrails.py` · **~20 min**

1. Ejecuta el lab.
2. Mira cómo los guardrails pre/post bloquean inyecciones y salidas demasiado largas.
3. Comprobación: `python labs/run_exercises.py --module m6`

---

## M7 — Confianza, verificación y provenance

**Lab:** `lab07_receipt_verify.py` · **~20 min** · requiere `[hub-lite]`

1. Ejecuta el lab.
2. Verifica un receipt firmado; altera el precio y observa cómo la verificación falla.
3. Comprobación: `python labs/run_exercises.py --module m7`

---

## M8 — Economía de la orquestación (avanzado)

**Lab:** `lab08_metered_economy.py` · **~45 min** · requiere `[sandbox]`

1. Ejecuta el lab.
2. channel → invoke → **total spent** → receipt.
3. Lee [ai-market-protocol-v1.md](https://github.com/alexar76/aicom/blob/main/docs/ai-market-protocol-v1.md) (402, channels).
...
```bash
python labs/run_exercises.py                 # comprobaciones tras cada lab
...
3. Producción: [observability-langsmith.md](https://github.com/alexar76/aicom/blob/main/docs/observability-langsmith.md)
...
3. [USER_GUIDE.es.md](https://github.com/alexar76/aicom/blob/main/docs/USER_GUIDE.es.md) — primeros 15 minutos en Admin
...
Factory: [FAQ.es.md](https://github.com/alexar76/aicom/blob/main/docs/FAQ.es.md)
