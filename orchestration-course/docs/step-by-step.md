...
7. [Module M5 — State, memory, context](#module-m5--state-memory-context)
8. [Module M6 — Guardrails & safety](#module-m6--guardrails--safety)
9. [Module M7 — Trust, verification, provenance](#module-m7--trust-verification-provenance)
10. [Module M8 — Economics of orchestration (advanced)](#module-m8--economics-of-orchestration-advanced)
11. [Optional LLM demo (Module M1)](#optional-llm-demo-module-m1)
12. [Exercises & certificate](#exercises--certificate)
13. [Module M9 — Observability in every lab](#module-m9--observability-in-every-lab)
14. [Bridge: from course to AI-Factory](#bridge-from-course-to-ai-factory)
15. [Self-check checklist](#self-check-checklist)
...
1. Open [course site](https://alexar76.github.io/aimarket-courses/orchestration-course/) → **Open in Colab** on any lab.
...
3. Read the `Trace` block the lab prints at the end — the orchestration is logged, not magic.
4. Contrast the shapes in the trace: ordered `pipeline_stage` events (sequential) vs the `fanout`/`gather` pair (parallel) vs a single `route` (router). `Parallel` here is deterministic (no real threads) — the distinction is **structural**, visible in the events, not in wall-clock.
...
4. Cross-read: [hub-integration-guide.md](https://github.com/alexar76/aicom/blob/main/docs/hub-integration-guide.md) § discovery.
...
4. Read [ai-market-protocol-v1.md](https://github.com/alexar76/aicom/blob/main/docs/ai-market-protocol-v1.md) § HTTP 402 and channels.
...
4. Production path: [observability-langsmith.md](https://github.com/alexar76/aicom/blob/main/docs/observability-langsmith.md) — factory OTel + LangSmith (not required for course).
...
3. **Admin path** — [USER_GUIDE.md](https://github.com/alexar76/aicom/blob/main/docs/USER_GUIDE.md) § “Your first 15 minutes”:
   - `/admin/login`
...
5. **List on Hub** — [ecosystem-architecture.md](https://github.com/alexar76/aicom/blob/main/docs/ecosystem-architecture.md) — product → capability → invoke.
