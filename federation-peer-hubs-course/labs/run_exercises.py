...
from courselib.i18n import SUPPORTED

# CI / instructors: COURSE_USE_SOLUTIONS=1 loads reference implementations.
if os.environ.get("COURSE_USE_SOLUTIONS", "").strip().lower() in ("1", "true", "yes", "on"):
    from courselib.exercise_solutions import EXERCISES, run_all
else:
    from courselib.exercises import EXERCISES, run_all


def main() -> int:
    p = argparse.ArgumentParser(description="Run Federation & Peer Hubs exercises")
    ...
    args = p.parse_args()
    if args.skip_check and os.environ.get("ACADEMY_ALLOW_SKIP_CHECK", "").strip() not in ("1", "true", "yes", "on"):
        print("--skip-check disabled. Pass exercises, or set ACADEMY_ALLOW_SKIP_CHECK=1 for demos.", file=sys.stderr)
        return 2
