...
def _exercises_passed() -> bool:
    import os

    if os.environ.get("COURSE_USE_SOLUTIONS", "").strip().lower() in ("1", "true", "yes", "on"):
        from courselib.exercise_solutions import all_passed, run_all
    else:
        from courselib.exercises import all_passed, run_all
    return all_passed(run_all())
...
def render_html(
    ...
    *,
    ...
    labels = {
        ...
        "fr": ("Certificat de réussite", "Décerné à", "Date", "ID du diplôme", "Modules maîtrisés"),
        "zh": ("结业证书", "授予", "日期", "证书编号", "已掌握模块"),
    }
  ...
  <style>
    * {{ box-sizing: border-box; }}
    ...
    h1 {{
      ...
      margin: 0.4rem 0 0.2rem; color: #e8edf7;
      background: linear-gradient(90deg, #fff, #b8c9ff);
    ...
    @media print {{
      body {{ background: #fff !important; color: #111 !important; display: block !important; min-height: auto !important; }}
      .cert {{
        box-shadow: none !important; border-color: #333 !important; background: #fff !important; color: #111 !important;
        width: 100% !important; max-width: 100% !important; margin: 0 !important; page-break-inside: avoid;
      }}
      ...
      .eyebrow, .label, .course, .meta, .footer {{ color: #444 !important; }}
      .name, .meta strong {{ -webkit-text-fill-color: #111 !important; color: #111 !important; }}
      .pill {{ background: #f4f4f4 !important; border-color: #888 !important; color: #111 !important; }}
      .seal {{ border-color: #333 !important; color: #111 !important; background: #fff !important; }}
    }}
  </style>
</head>
<body>
...
def write_certificate(path: Path, name: str, *, lang: str = "en", require_exercises: bool = True) -> Path:
    if require_exercises and not _exercises_passed():
        import os

        if os.environ.get("COURSE_USE_SOLUTIONS", "").strip().lower() in ("1", "true", "yes", "on"):
            from courselib.exercise_solutions import run_all
        else:
            from courselib.exercises import run_all
        results = run_all()
