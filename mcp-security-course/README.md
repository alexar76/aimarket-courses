...
> `courselib/warden.py` is a **teaching port**, written to be read in one sitting. The shipping
> firewall is [`@aimarket/warden`](https://github.com/alexar76/warden) — a zero-dependency
> TypeScript package you can put in front of your own MCP host — and the labs point at its source so
> you can compare the real rule table with the one you build here.
