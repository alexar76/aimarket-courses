...
GITHUB_PAGES = _CFG["github_pages"]
REPO_SUBPATH = (_CFG.get("repo_subpath") or "").strip("/")
QUICKSTART_COMMANDS = _CFG["quickstart_commands"]
LABS = [{k: v for k, v in lab.items() if k != "sandbox"} for lab in _CFG["labs"]]
MODULE_ORDER = _CFG["modules"]
_COURSE_TITLE = _CFG["title"]
_COURSE_DESCRIPTION = _CFG["description"]
...
def _exercise_modules() -> list[str]:
    if str(ROOT) not in sys.path:
        sys.path.insert(0, str(ROOT))
    try:
        from courselib.exercises import EXERCISES

        return sorted(EXERCISES.keys(), key=lambda k: (len(k), k))
    except Exception:
...
def _colab_url(stem: str) -> str:
    ...
    nb = f"{prefix}notebooks/{stem}.ipynb"
    return (
...
def _build_payload(catalogs: dict[str, dict]) -> dict:
    ...
    return {
        ...
        "exerciseModules": _exercise_modules(),
        "labs": labs,
...
def _write_assets() -> None:
    css = textwrap.dedent(
        ...
        * { box-sizing: border-box; }
        ...
        .wow-stage {
          ...
          border: 1px solid var(--border-strong);
          background:
            radial-gradient(ellipse at 50% 40%, rgba(103,232,249,0.08), transparent 55%),
            radial-gradient(ellipse at 50% 70%, rgba(192,132,252,0.08), transparent 50%),
            rgba(0,0,0,0.28);
          overflow: hidden;
        }
        #wow-canvas { display: block; width: 100%; height: auto; }
        .wow-caption {
          margin: 0;
    ...
    progress_key = f"{ROOT.name}-progress-v1"
    for name in ("progress.js", "certificate.js", "app.js", "wow.js"):
        src = (SITE_ASSETS / name).read_text(encoding="utf-8")
        if name == "progress.js":
            src = src.replace("__STORAGE_KEY__", progress_key)
        (SITE / "assets" / name).write_text(src, encoding="utf-8")
    (SITE / "assets" / "style.css").write_text(css, encoding="utf-8")
    # Webfonts, copied verbatim so the published site serves them itself. The head used to
    # <link> them from fonts.googleapis.com, which handed Google the address of every student
    # who opened a lesson. Populate with scripts/selfhost_google_fonts.py.
    fonts_src = SITE_ASSETS / "fonts"
    if fonts_src.is_dir():
        fonts_dest = SITE / "fonts"
...
def _write_index(*, asset_version: str) -> None:
    html = textwrap.dedent(
        ...
        <head>
          ...
          <title>{_COURSE_TITLE}</title>
          <meta name="description" content="{_COURSE_DESCRIPTION}" />
        ...
        <body>
          <div class="wrap">
            <header class="hero">
              <div class="hero-top">
                <div class="eyebrow">Python · EN / RU / ES / FR / ZH</div>
                <div class="hero-badge" id="hero-badge"></div>
              </div>
              <h1 id="title">{_COURSE_TITLE}</h1>
              <p class="tagline" id="tagline"></p>
              <div class="wow-stage" aria-hidden="true">
                <canvas id="wow-canvas" width="640" height="300"></canvas>
                <p class="wow-caption" id="wow-caption"></p>
              </div>
              ...
              <div class="toolbar">
                ...
                <a class="btn primary" href="https://github.com/{GITHUB_REPO}">GitHub ↗</a>
              </div>
          ...
          <script src="assets/wow.js?v={asset_version}"></script>
...
def main() -> None:
    ...
    global SITE
    parser = argparse.ArgumentParser(description="Build course GitHub Pages site")
    parser.add_argument(
        "--out",
        type=Path,
        default=None,
        help="Output directory (default: ./site under course root)",
    )
    args = parser.parse_args()
    if args.out:
        SITE = args.out.resolve()
    ...
    _write_assets()
    asset_names = ("app.js", "progress.js", "certificate.js", "wow.js")
    asset_version = hashlib.sha256(
        b"".join((SITE_ASSETS / name).read_bytes() for name in asset_names)
    ).hexdigest()[:10]
